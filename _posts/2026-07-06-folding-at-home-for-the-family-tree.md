...
Second, even where the data is fine, plenty of these samples have never been placed on our tree. The flagship collection is in good shape: most of the 1000 Genomes Project is already sitting on the Decoding Us tree. But that is the one I got to first because it is the largest and best known. Behind it sits a long tail of smaller studies I simply have not reached yet: the Human Genome Diversity Project, the 90 Han Chinese genomes, the Gambian Genome Variation Project, and many more like them. Each of those is a set of Y chromosomes that belong on specific branches, with specific ancestry profiles, sitting unread in an archive. Add them all up and you have a genuinely useful map of human genetic history that is only partly drawn.
...
Imagine every public sample that matters to our field placed on the *same* modern tree, with the *same* pipeline, all comparable to each other and to your own results. The 1000 Genomes Project is largely there already. Now extend that same treatment to the diversity panels and the smaller regional studies (the Human Genome Diversity Project, the 90 Han Chinese, the Gambian Genome Variation Project, and their kin), plus every published ancient genome, each given an honest Y and mitochondrial placement on T2T coordinates and an ancestry profile computed the same way yours was. That is a reference map of human paternal and maternal lineages, drawn to a consistent standard, that only partly exists today because reaching the long tail is more compute than one person can spare.
